                ...
                <div className=" flex items-center w-full justify-between ">
                    <Link href="/" legacyBehavior prefetch={false}>
                        <a className=' flex items-center gap-x-2 text-black '>
                            ...
                            <span className='hidden sm:block text-xs sm:text-base'>Emprende con Notion</span>
                    ...
                    <div className="hidden sm:flex sm:gap-x-4">
                        {navigation.map((item) => (
                            <Link href={item.href} key={item.name} legacyBehavior prefetch={false}>
                                <a key={item.name} className={`text-xs block h-full text-black  hover:text-bid-terracotta ${item.href === pathname ? 'text-bid-terracotta' : ''}`}>
                                    {item.name}
                                </a>
                            </Link>
                        ))}
                            ...
                            <div className="space-y-2 py-6">
                                {navigation.map((item) => (
                                    <Link href={item.href} key={item.name} legacyBehavior prefetch={false}>
                                        <a
                                            key={item.name}
                                            className="-mx-3 block rounded-sm px-3 py-2 leading-7  hover:text-bid-terracotta"
                                            onClick={() => setMobileMenuOpen(false)}
                                        >
                                            {item.name}
                                        </a>
